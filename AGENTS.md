...
| Атрибут | Тип | Описание |
|---|---|---|
| `caption_lines_number` | int | Общий предел строк заголовка для всех колонок таблицы. View-атрибут. |
| `caption_lines` | int | То же самое для отдельной колонки, перекрывает общий `caption_lines_number`. |

Значения одинаковые: `1` (по умолчанию) – прежнее поведение: одна строка, дальше многоточие;
`0` – сколько угодно строк; `N` – не больше N строк, дальше многоточие. При значении, отличном
от `1`, высота шапки перестаёт быть фиксированной и растёт под содержимое (`min-height` прежний),
у заголовка появляются вертикальные отступы.

```sql
SELECT
    @type = 'table',
    @caption_lines_number = 2,
    id @{ caption = 'Идентификатор', caption_lines = 1 },
    ...
```
