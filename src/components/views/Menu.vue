...
<template>
  <div
    :class="[
      'menu_container',
      { menu_container__centered: isCentered, 'is-mobile': $isMobile },
    ]"
  >
...
import MenuEntry, {
  ...
  IMenuLink,
  Badge,
} from '@/components/views/menu/MenuEntry.vue'
...
export default class UserViewMenu extends mixins<EmptyBaseUserView>(
  BaseUserView,
) {
  ...
  private convertNewMenuEntry(
    ...
    if ('content' in entry) {
      ...
      const hasBadge =
        typeof entry.badge === 'object' &&
        entry.badge !== null &&
        'value' in entry.badge
      let badge: Badge | undefined
      if (hasBadge) {
        const badgeRaw = entry.badge as { value?: unknown; variant?: unknown }
        badge = {
          value: badgeRaw.value,
          variant: colorVariantFromAttribute(
            badgeRaw?.variant,
            bootstrapVariantAttribute('danger'),
          ),
        }
      }
    ...
    return this.uv
      .mapVisibleRows((row) => {
        ...
        if (rawMenu instanceof Array) {
          ...
          const ret = this.convertNewMenuEntry(
            rawMenu as Record<string, unknown>,
          )
          if (ret === null) {
            return []
          } else {
  ...
  private buildOldMenu(): MenuValue[] | string {
    ...
    const categories = new Map<
      string,
      { index: number; content: IMenuLink[] }
    >()
    this.uv.rows!.forEach((row, rowI) => {
      ...
      const categoryName = valueToPunnedText(
        categoryColumnInfo.valueType,
        categoryCell,
      )
      ...
      const buttonName = valueToPunnedText(
        buttonColumnInfo.valueType,
        buttonCell,
      )
      const buttonAttrs = buttonCell.attributes || {}
      const getButtonAttr = (name: string) =>
        tryDicts(name, buttonAttrs, buttonsAttrs, rowAttrs, viewAttrs)
    ...
    return Array.from(categories.entries())
      .sort(([nameA, a], [nameB, b]) => a.index - b.index)
      .map(([name, x]) => ({ name, content: x.content }))
  }
