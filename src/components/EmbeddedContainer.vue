...
export default class EmbeddedContainer extends Vue {
  ...
  private maybeInitialize(msg: object): boolean {
    if ((msg as any).name === 'ready') {
      ...
      if (
        typeof reqMsg.request === 'object' &&
        reqMsg.request?.type === 'ready'
      ) {
  ...
  private handleApi0(msg: object): boolean {
    ...
    if (
      legacyMsg.name === 'changeHeight' &&
      typeof legacyMsg.payload === 'number' &&
      legacyMsg.payload >= 0
    ) {
        ...
        updateValue: (request: Embedded.IUpdateValueRequestData) => {
          if (!this.isControl) {
            return {
              ...
              message: 'Embedded container is not a control',
            }
  ...
  private iframeEventHandler(event: MessageEvent<unknown>) {
    if (
      event.source !==
      (this.$refs.iframe as HTMLIFrameElement | undefined)?.contentWindow
    )
      return
  ...
  private processRequest<R extends { type: string }>(
    msg: Embedded.IRequest<R>,
    handlers: Handlers<R>,
  ) {
    ...
    if (
      ...
      try {
        ...
        if ('result' in result) {
          ...
          this.sendMessage(response)
          console.error(
            `Request from embedded container failed, id "${msg.id}", type "${request.type}": ${result.message}`,
          )
        }
      } catch (e) {
        const response: Embedded.IResponseError<Embedded.CommonError> = {
        ...
        this.sendMessage(response)
        console.error(
          `Request from embedded container failed, id "${msg.id}", type "${request.type}":`,
          e,
        )
      }
    } else {
      ...
      this.sendMessage(response)
      console.error(
        `Unknown request type received from embedded container, id "${msg.id}", type "${request.type}"`,
      )
    }
  }
