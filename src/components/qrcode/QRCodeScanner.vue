...
<template>
  <span>
    <BarCode v-if="textInput" @scanned="onDecode" />
    <StreamBarcodeReader v-else @decode="onDecode" />

    <div v-if="result.length > 0" class="decode-result">
      <b-button block variant="success" @click="sendList">
        {{ $t('paste_data') }}
      </b-button>
      <strong>{{ $t('scan_result') }}:</strong>
      <ol reversed>
        <li v-for="(value, i) in result.slice().reverse()" :key="i">
          {{ value.value }}
        </li>
      </ol>
    </div>
...
@Component({
  components: {
    StreamBarcodeReader: async () =>
      (await import('vue-barcode-reader')).StreamBarcodeReader,
...
export default class QRCodeScanner extends mixins(BaseEntriesView) {
  ...
  private async onDecode(content: string) {
    ...
    try {
      ...
      const entries = await this.fetchEntriesByIds(this.entries, [
        currentContent.id,
      ])
  ...
  private async dispatchByContent(currentContent: IQRCode) {
    ...
    if ('links' in this.link) {
      link =
        this.link.links[currentContent.entity.schema]?.[
          currentContent.entity.name
        ]
      if (link === undefined) {
