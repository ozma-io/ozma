...
import StagingChangesState from "@/state/staging_changes"
...
export const currentStaging = async (apiFunc: ((_1: string, ..._2: any[]) => Promise<any>), ...args: any[]): Promise<any> => {
    if (store.state.auth.current === null) {
        throw new Error("No authentication token to renew")
    }
