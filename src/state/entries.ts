...
export type UpdateSearchNode =
  | IUpdateSearchNodeOK
  | IUpdateSearchNodeError
  | IUpdateSearchNodePending

export interface ISearchNodeOK extends ISearchNodeBase, IUpdateSearchNodeOK {
  children: SearchNode[]
}

export interface ISearchNodeError
  extends ISearchNodeBase,
    IUpdateSearchNodeError {}

export interface ISearchNodePending
  extends ISearchNodeBase,
    IUpdateSearchNodePending {
  children: SearchNode[]
}
...
const punToText = (pun: unknown, id: unknown, punType: ValueType) =>
  pun === null ? `${id}` : valueToText(punType, pun)

// Add new node to the search tree.
const insertSearchNode = (
  node: SearchNode,
  search: string,
  limit: number,
  pending: Promise<boolean>,
  ...
  if (node.search === search) {
    ...
    const [newSubnodeChildren, newChildren] = R.partition(
      (subnode) => search.indexOf(subnode.search) !== -1,
      node.children,
    )
...
const updateSearchNode = (
  node: SearchNode,
  search: string,
  update: UpdateSearchNode,
): boolean => {
  if (node.search === search) {
...
export class PartialEntries {
  ...
  insert(search: string, limit: number, pending: Promise<boolean>) {
    ...
    if (this.searchTree === null) {
      ...
      !insertSearchNode(this.searchTree, lowerSearch, limit, pending)
    ) {
  ...
  update(search: string, update: UpdateSearchNode) {
    if (this.searchTree === null) {
      ...
      !updateSearchNode(this.searchTree, search.toLowerCase(), update)
    ) {
...
const fetchEntriesByEntity = async (
  context: ActionContext<IEntriesState, {}>,
  ref: IEntityRef,
  search: string,
  offset: number,
  limit: number,
  ...
  const likeSearch =
    search === '' ? '%' : `%${search.replaceAll(/\\|%|_/g, '\\$&')}%` // Escape characters.
  ...
  const res = (await context.dispatch(
    ...
    {
      func: (api: FunDBAPI) =>
        api.getAnonymousUserView(query, { search: likeSearch }, { chunk }),
    },
    { root: true },
  )) as IViewExprResult
  const mainType = res.info.columns[1].valueType
  const entries = Object.fromEntries(
    res.result.rows.map<[number, string]>((row) => {
...
const fetchEntriesByDomain = async (
  context: ActionContext<IEntriesState, {}>,
  referencedBy: IReferencedField,
  search: string,
  offset: number,
  ...
  const res = (await context.dispatch(
    'callApi',
    {
      func: (api: FunDBAPI) =>
        api.getDomainValues(
          referencedBy.field,
          referencedBy.rowId ?? undefined,
          opts,
        ),
    },
    { root: true },
  )) as IDomainValuesResult
  const entries = Object.fromEntries(
    res.values.map<[number, string]>((row) => {
...
const fetchEntriesByOptionsView = async (
  context: ActionContext<IEntriesState, {}>,
  optionsView: IQuery,
  search: string,
  offset: number,
    ...
    'callApi',
    {
      func: (api: FunDBAPI) =>
        api.getNamedUserView(
          source.ref,
          optionsView.args.args ?? undefined,
          opts,
        ),
    },
    { root: true },
  ...
  const idColumnIndex = res.info.columns.findIndex(
    (column) => column.name === 'value' || column.name === 'id',
  ) // "id" is deprecated.
  const nameColumnIndex = res.info.columns.findIndex(
    (column) => column.name === 'pun' || column.name === 'name',
  ) // "name" is deprecated.
  if (idColumnIndex === -1 || nameColumnIndex === -1) {
    throw new Error(
      'User view for reference constraint must have columns `value` and `pun`',
    )
  }
...
const fetchEntries = async (
  ...
  switch (ref.fetchBy) {
    ...
    case 'options_view':
      return fetchEntriesByOptionsView(
        context,
        ref.optionsView,
        search,
        offset,
...
const fetchEntriesByDomainByIds = async (
  context: ActionContext<IEntriesState, {}>,
  referencedBy: IReferencedField,
  ids: RowId[],
...
const fetchEntriesByEntityByIds = async (
  context: ActionContext<IEntriesState, {}>,
  entity: IEntityRef,
  ids: RowId[],
): Promise<Record<string, string>> => {
...
const fetchEntriesByOptionsViewByIds = async (
  context: ActionContext<IEntriesState, {}>,
  optionsView: IQuery,
  ids: RowId[],
): Promise<Record<string, string>> => {
...
const fetchEntriesByOptionsViewOrEntityByIds = async (
  context: ActionContext<IEntriesState, {}>,
  optionsView: IQuery,
  entity: IEntityRef | null,
  ids: RowId[],
) => {
  const optionsViewEntries = await fetchEntriesByOptionsViewByIds(
    context,
    optionsView,
    ids,
  )
  const optionsViewEntriesIds = Object.keys(optionsViewEntries).map((key) =>
  ...
  if (ids.length === optionsViewEntriesIds.length) {
    ...
    if (entity) {
      ...
      entityEntries = await fetchEntriesByEntityByIds(
        context,
        entity,
        notFoundInOptionsViewIds,
      )
    }

    if (
      optionsViewEntriesIds.length + Object.keys(entityEntries).length ===
      ids.length
    ) {
      return { ...optionsViewEntries, ...entityEntries }
    } else {
      const foundIds = [
        ...optionsViewEntriesIds,
        ...Object.keys(entityEntries).map((key) => Number.parseInt(key, 10)),
      ]
      ...
      const notFoundEntries = Object.fromEntries(
        notFoundIds.map((id) => [id, String(id)]),
      )
...
const fetchEntriesByIds = async (
  ...
  switch (ref.fetchBy) {
    ...
    case 'options_view':
      return fetchEntriesByOptionsViewOrEntityByIds(
        context,
        ref.optionsView,
        ref.referencedTo,
        ids,
...
export const getReferenceInfo = (
  ...
  columnI: number,
  rowId: number | null,
): { referenceEntity: IEntityRef; entries: IEntriesRefByDomain } | null => {
...
const entriesModule: Module<IEntriesState, {}> = {
  ...
  mutations: {
    initPartialEntries: (
      state,
      args: { ref: EntriesRef; reference: ReferenceName },
    ) => {
      const partial = new PartialEntries()
      state.current.entries.createResource(
        args.ref,
        args.reference,
        partial,
        undefined,
    ...
    insertSearchNode: (
      ...
      state.current
        .getEntries(args.ref)!
        .insert(args.search, args.limit, args.pending)
    },
    updateSearchNode: (
      state,
      args: { ref: EntriesRef; search: string; update: UpdateSearchNode },
    ) => {
    ...
    addSingleEntry: (
      state,
      args: { ref: EntriesRef; id: number; main: string },
    ) => {
      state.current.getEntries(args.ref)!.addSingleEntry(args.id, args.main)
    },
    updatePendingSingleEntry: (
      state,
      args: { ref: EntriesRef; id: number; pending: Promise<Entries> | null },
    ) => {
      state.current
        .getEntries(args.ref)!
        .updatePendingSingleEntry(args.id, args.pending)
    },
    removePendingSingleEntry: (
      state,
      args: { ref: EntriesRef; id: number },
    ) => {
      state.current.getEntries(args.ref)!.removePendingSingleEntry(args.id)
    },
    addEntriesConsumer: (
      state,
      { ref, reference }: { ref: EntriesRef; reference: ReferenceName },
    ) => {
  ...
  actions: {
    ...
    getEntriesByIds: (
      context,
      args: { reference: ReferenceName; ref: EntriesRef; ids: RowId[] },
    ): Promise<Entries> => {
      ...
      if (requestedIds.length !== 0) {
        ...
        pending.ref = (async () => {
          ...
          for (const id of requestedIds) {
            const pun = puns[id]
            const currPending =
              state.current.entries.get(ref)?.pendingSingleEntries[id]
            if (currPending !== pending.ref) {
              console.warn(
                `Pending operation cancelled, requested ids ${requestedIds}`,
              )
              continue
            }

            if (pun === undefined) {
              commit('updatePendingSingleEntry', {
                ref,
                reference,
                id,
                pending: null,
        ...
        if (oldResource === undefined) {
          ...
          if (ref.fetchBy === 'domain' && ref.referencedBy !== null) {
            void dispatch('entities/getEntity', ref.referencedBy.field.entity, {
              root: true,
            })
          }
      ...
      return Promise.all(awaitedIds.map(([id, promise]) => promise)).then(
        (newEntries) => {
    ...
    getEntries: async (
      ...
      if (oldResource !== undefined) {
        ...
        if (oldCurrent !== state.current) {
          throw new CancelledError(
            `Pending entries got cancelled, ref ${JSON.stringify(ref)}`,
          )
        }
      ...
      pending.ref = (async () => {
        ...
        if (
          currNode?.status !== 'pending' ||
          currNode.pending !== pending.ref
        ) {
          throw new CancelledError(
            'Pending entries got cancelled, ref ' + JSON.stringify(ref),
          )
