...
export interface ICombinedInsertEntityResult
  extends IInternalInsertEntityOp,
    IInsertEntityResult {}

export interface ICombinedUpdateEntityResult
  extends IInternalUpdateEntityOp,
    IUpdateEntityResult {
  id: RowId
}

export interface ICombinedDeleteEntityResult
  extends IDeleteEntityOp,
    IDeleteEntityResult {}

export type CombinedTransactionResult =
  | ICombinedInsertEntityResult
  | ICombinedUpdateEntityResult
  | ICombinedDeleteEntityResult

type InternalTransactionOp =
  | IInternalInsertEntityOp
  | IInternalUpdateEntityOp
  | IDeleteEntityOp
...
export class CurrentChanges {
  ...
  get addedCount() {
    return Object.values(this.scopes).reduce(
      (prev, scope) => prev + scope.addedCount,
      0,
    )
  }
  ...
  isScopeEmpty(scope: ScopeName) {
    return (
      this.updatedCount === 0 &&
      this.deletedCount === 0 &&
      (this.scopes[scope]?.addedCount ?? 0) === 0
    )
  }
  ...
  cleanupEntity(ref: IEntityRef, entityChanges: IEntityChanges) {
    if (
      Object.keys(entityChanges.updated).length === 0 &&
...
const getEntityInfo = async (
  context: ActionContext<IStagingState, {}>,
  ref: IEntityRef,
): Promise<IEntity> => {
...
const getFieldInfo = async (
  ...
  if (fieldInfo === undefined) {
    throw new Error(
      `No field info for field ${ref.entity.schema}.${ref.entity.name}.${ref.name}`,
    )
  }
  return fieldInfo
}
...
const getEmptyValues = (scope: ScopeName, entity: IEntity): UpdatedValues => {
  return Object.fromEntries(
    Object.entries(entity.columnFields)
      .filter(
        ([name, info]) => !(info.isNullable || info.defaultValue !== undefined),
      )
      .map(([name, info]) => {
...
const entityChangesToOperations = async (
  context: ActionContext<IStagingState, {}>,
  scope: ScopeName | null,
  errorOnIncomplete: boolean,
): Promise<InternalTransactionOp[]> => {
  const nestedOps = await Promise.all(
    Object.entries(context.state.current.changes).map(
      async ([schemaName, entities]) => {
        const ret = await Promise.all(
          Object.entries(entities).map(async ([entityName, entityChanges]) => {
            try {
              const entity = {
                schema: schemaName,
                name: entityName,
              ...
              const updated = mapMaybe(([updatedIdStr, updatedFields]) => {
                const fields = mapMaybe(([name, change]) => {
                  if (change.value === undefined) {
                    if (!errorOnIncomplete) {
                      ...
                      throw new Error(
                        `Value for ${name} didn't pass validation`,
                      )
                    }
                  } else {
              ...
              const added = mapMaybe(([addedIdStr, addedFields]) => {
                ...
                const fields = Object.entries(addedFields.values).map(
              ...
              const deleted = Object.keys(entityChanges.deleted).map(
                (deletedIdStr) => {
                  return {
                    type: 'delete',
                    entity,
              ...
              throw new Error(
                `Invalid value for ${schemaName}.${entityName}: ${e}`,
              )
            }
          }),
        )
...
): Record<FieldName, unknown> => {
  return Object.fromEntries(
    Object.entries(entries).map(([name, value]) => [
      name,
      serializeValue(entityInfo.columnFields[name].fieldType, value),
    ]),
  )
}

const internalOpToTransactionOp = (
  op: InternalTransactionOp,
): TransactionOp => {
  if (op.type === 'insert') {
    return {
      type: 'insert',
      entity: op.entity,
      fields: serializeValues(op.entityInfo, op.fields),
    }
...
const stagingModule: Module<IStagingState, {}> = {
  ...
  mutations: {
    ...
    addEntry: (
      state,
      params: { scope: ScopeName; entityRef: IEntityRef; entityInfo: IEntity },
    ) => {
    ...
    resetAddedEntry: (
      state,
      params: { entityRef: IEntityRef; id: AddedRowId },
    ) => {
      const { entityRef, id } = params
    ...
    resetDeleteEntry: (
      state,
      { entityRef, id }: { entityRef: IEntityRef; id: RowId },
    ) => {
      if (state.currentSubmit !== null) {
        throw new Error('Resetting changes is forbidden while submitting')
      }
    ...
    setHandler: (
      state,
      { key, handler }: { key: StagingKey; handler: IStagingEventHandler },
    ) => {
  ...
  actions: {
    updateField: async (
      ...
      const value = state.current.getOrCreateChanges(args.fieldRef.entity)
        .updated[args.id][args.fieldRef.name]
      Object.values(state.handlers).forEach((handler) =>
        handler.updateField(args.fieldRef, args.id, value, args.meta),
      )
    },
    addEntry: async (
      context,
      ...
      const newId =
        state.nextAddedId === 0 ? maxNextAddedId : state.nextAddedId - 1
      await checkCounters(context)

      Object.values(state.handlers).forEach((handler) =>
        handler.addEntry(args.entityRef, newId, args.meta),
      )
    ...
    setAddedField: async (
      ...
      // Vuex is stupid, so we need to re-fetch the value.
      const value = state.current.getOrCreateChanges(args.fieldRef.entity)
        .added[args.id].values[args.fieldRef.name]
      Object.values(state.handlers).forEach((handler) =>
        handler.setAddedField(args.fieldRef, args.id, value, args.meta),
      )
    },
    deleteEntry: async (
      context,
      args: { entityRef: IEntityRef; id: RowId; meta?: unknown },
    ) => {
      const { state, commit } = context
      ...
      // Vuex is stupid, so we need to re-fetch the value.
      Object.values(state.handlers).forEach((handler) =>
        handler.deleteEntry(args.entityRef, args.id, args.meta),
      )
    },
    ...
    reset: (context) => {
      ...
      for (const [schemaName, schemaChanges] of Object.entries(
        state.current.changes,
      )) {
        for (const [entityName, entityChanges] of Object.entries(
          schemaChanges,
        )) {
          const entityRef = {
            schema: schemaName,
          ...
          for (const [rawId, updated] of Object.entries(
            ...
            for (const fieldName of Object.keys(updated)) {
              ...
              Object.values(state.handlers).forEach((handler) =>
                handler.resetUpdatedField(fieldRef, id),
              )
            }
          }
          ...
          for (const rawId of Object.keys(entityChanges.added)) {
            const id = Number(rawId)
            Object.values(state.handlers).forEach((handler) =>
              handler.resetAddedEntry(entityRef, id),
            )
          }
          ...
          for (const rawId of Object.keys(entityChanges.deleted)) {
            const id = Number(rawId)
            Object.values(state.handlers).forEach((handler) =>
              handler.resetDeleteEntry(entityRef, id),
            )
          }
        }
      }
    ...
    resetUpdatedField: async (
      ...
      if (!args.dontNotify) {
        Object.values(state.handlers).forEach((handler) =>
          handler.resetUpdatedField(args.fieldRef, args.id, args.meta),
        )
      }
    ...
    resetAddedEntry: async (
      ...
      if (!args.dontNotify) {
        Object.values(state.handlers).forEach((handler) =>
          handler.resetAddedEntry(args.entityRef, args.id, args.meta),
        )
      }
    ...
    resetDeleteEntry: async (
      ...
      if (!args.dontNotify) {
        Object.values(state.handlers).forEach((handler) =>
          handler.resetDeleteEntry(args.entityRef, args.id, args.meta),
        )
      }
    ...
    clearAdded: async (
      ...
      for (const [schema, entities] of Object.entries(state.current.changes)) {
        for (const [entity, entityChanges] of Object.entries(entities)) {
          for (const [addedIdStr, addedEntry] of Object.entries(
            entityChanges.added,
          )) {
            if (scope === undefined || scope === addedEntry.scope) {
              ...
              // eslint-disable-next-line
              await dispatch('resetAddedEntry', {
    ...
    clearUnchanged: async (context, ops: CombinedTransactionResult[]) => {
      ...
      await Promise.all(
        ops.map(async (op) => {
          const entityChanges =
            state.current.changes[op.entity.schema][op.entity.name]
          if (op.type === 'insert') {
            if (op.id !== null) {
              Object.values(state.handlers).forEach((handler) =>
                handler.commitAddedEntry(op.entity, op.internalId, op.id!),
              )
              const addedValues = entityChanges.added[op.internalId]
              await Promise.all(
                Object.entries(addedValues.values).map(
                  async ([fieldName, newValue]) => {
                    const maybeOldValue = op.fields[fieldName]
                    const valueType =
                      op.entityInfo.columnFields[fieldName].valueType
                    if (
                      !valueEquals(valueType, maybeOldValue, newValue.value)
                    ) {
              ...
              Object.entries(updatedValues).map(
                async ([fieldName, newValue]) => {
                  ...
                  if (valueEquals(valueType, maybeOldValue, newValue.value)) {
                    await dispatch('resetUpdatedField', {
                      fieldRef: { entity: op.entity, name: fieldName },
                      id: op.id,
                      dontNotify: true,
                    })
                  }
            ...
            await dispatch('resetDeleteEntry', {
              entityRef: op.entity,
              id: op.id,
              dontNotify: true,
            })
    ...
    submit: async (
      ...
      try {
        ops = await entityChangesToOperations(
          context,
          params.scope ?? null,
          params.errorOnIncomplete ?? false,
        )
      } catch (e) {
        commit(
          'errors/pushError',
          { key: errorKey, error: String(e) },
          { root: true },
        )
        throw e
      }
      ...
      const submit = (async (): Promise<ISubmitResult> => {
        ...
        if (!(result instanceof Error)) {
          ...
          if (retLink !== null) {
            ...
            commit(
              'errors/setError',
              { key: errorKey, error: result.message },
              { root: true },
            )
          }
          throw result
